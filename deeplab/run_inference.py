...
flags.DEFINE_string('structure_match', 'Parotid_L_BASE',
...
def create_rtstruct(RS_File, im_mask_ax, im_mask_sag, im_mask_cor):
    ...
    ss.StructureSetName = FLAGS.structure_match + '_DLV3_' + datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")[0:19]
    ss.StructureSetLabel = datetime.datetime.now().strftime("%Y%m%d%H%M%S%f")[0:19]
...
def main(unused_argv):
    ...
    if RS_Files:
        ...
        for p_num in range(0, len(RS_Files)):
            ...
            for plane in planeList:
                ...
                if plane == 'axial':
                    ...
                    for i in range(0, infer_size):
                        ...
                        image = Image.open('hold.png')
                        r_im, seg = model.run(image)
                        im_mask_cor[strt+i,strt:stop,0:size[2]] = seg[:,0:size[2]]
                    ...
                    for i in range(0, infer_size):
                        ...
                        image = Image.open('hold.png')
                        r_im, seg = model.run(image)
                        im_mask_sag[strt:stop,strt+i,0:size[2]] = seg[:,0:size[2]]
            ...
            create_rtstruct(RS_Files[p_num], im_mask_ax, im_mask_ax, im_mask_ax)
